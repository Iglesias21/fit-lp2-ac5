...
#
# e-mails: ricardo.feliciano@aluno.faculdadeimpacta.com.br
...
class Email:
    ...
    def valida_email(email: str) -> bool:
        ...
        if not isinstance(email, str):
            raise TypeError('O e-mail deve ser uma string')
                        
        if (email.count('@') != 1):
            raise ValueError('O e-mail deve possuir apenas uma @')

        email_validacao = email.replace('@', '')
        email_validacao = email_validacao.replace('.', '')

        if not email_validacao.isalnum():
            raise ValueError('O e-mail deve conter apenas letra, números, ponto e @')
...
class Contato:
    ...
    def create_dump(self):
        ...
        """
        # expContato = {}
        # expContato["nome"] = json.dumps(self.nome, default=dumper, indent=2)
        # expContato["telefones"] = json.dumps(self.get_telefones(), default=dumper, indent=2, )
        # expContato["emails"] = json.dumps(self.get_emails(), default=dumper, indent=2)
        ...
        return expContato
        # return json.dumps(expContato, default=dumper, indent=2, ensure_ascii=True)
